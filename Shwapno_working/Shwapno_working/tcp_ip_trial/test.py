    ...
    try:
        if request.authorization:
            ...
            for userall in user_data2:
                if userall["user"] == request.authorization.Username:
...
def insert():
    ...
    try:
        if request.method == "POST": 
            ...
            if _device_no == "" or _temperature == "":
                res={"response": "Required field is empty!!!"}
                return jsonify(res)

            elif(cursor4.rowcount >0): 
                conn.commit()
                cursor4.close()
                conn.close()
...
def edit():
    ...
    try:
        if request.method == "POST":
            ...
            sql1 = "SELECT device_no FROM sensor WHERE device_no = %s"
            cursor2.execute(sql1,_device_no)
            ...
            if(cursor2.rowcount >0):
                ...
                res={"response": "no device found with the given device no!!!"}
                return jsonify(res)
